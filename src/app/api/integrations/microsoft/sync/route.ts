...
import { syncMeetingToMicrosoft, getMicrosoftConnectionStatus } from '@/lib/microsoft-calendar-service';
import { createAuditLog } from '@/lib/audit-service';

// POST /api/integrations/microsoft/sync - Sync meetings to Microsoft 365 Calendar
export async function POST(request: NextRequest) {
  try {
    ...
    const connection = await getMicrosoftConnectionStatus(tenantId);
    ...
    for (const meeting of meetings) {
      try {
        const result = await syncMeetingToMicrosoft(tenantId, {
          id: meeting.id,
          title: meeting.title,
